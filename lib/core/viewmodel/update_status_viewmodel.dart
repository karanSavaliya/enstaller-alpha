...
class UpdateStatusViewModel extends BaseModel {
  ...
  void textChange(String val) {
    ...
    _appointmentStatusList.forEach((element) {
      if (element.customerName.toLowerCase().contains(val.toLowerCase()) ||
          ...
          element.strCompanyName.toLowerCase().contains(val.toLowerCase())) {
  ...
  void initializeData() async {
    ...
    _appointmentStatusList = await _apiService.appointmentDataEventsbyEngineer(
        user.intEngineerId.toString(), user.intCompanyId);
    appointmentStatusList = _appointmentStatusList;
      ...
      for (int i = 0; i < selectedAppointments.length; i++) {
        ResponseModel response = await _apiService.updateAppointmentStatus(
            AppointmentStatusUpdateCredentials(
                strStatus: selectedStatus != AppStrings.enRoute
                    ? selectedStatus
                    : AppStrings.onRoute,
                intBookedBy: selectedAppointment.intBookedBy.toString(),
        ...
        if (i == selectedAppointments.length - 1) {
          ...
          if (response.statusCode == 1) {
            ...
            appointmentStatusList =
                await _apiService.appointmentDataEventsbyEngineer(
                    user.intEngineerId.toString(), user.intCompanyId);
            setState(ViewState.Idle);
          } else {
            setState(ViewState.Idle);
